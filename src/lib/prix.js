...
const pathComponents = `${location.origin}/privado/components` 
const pathPages = `${location.origin}/privado/pages`
const pathImg = `${location.origin}/privado/static/img`
const pathData = `${location.origin}/privado/static/data`
const pathStatic = `${location.origin}/privado/static`
...
function Router(Pages = {}, Data = {}, Config = {}) {



  async function customTagsPrivate(renderStyle, renderedHtml, renderState) {
    ...
    await Promise.all(
      tagElements.map(async (elem, i) => {
        const nomeDaTag = elem.tagName.toLowerCase().replace("comp-","")   
        ...
        const dataApp = {
          ...
          const pegarPaginaOculta = await useApi(
            `http://192.168.1.5:5500/privado/components/${nomeDaTag}.js`,
            {},
            "text"
          );
          const retonarPaginaOcultaEmFunção = Function("return " + pegarPaginaOculta)();
  ...
  async function routerState() {
    ...
    if (resultUrl === "erro") {
      ...
      if (typeof Pages[resultUrl] === "function") {
        ...
        const  paginasData = {Pages, Data}
        const pegarPaginaOculta = await useApi(`http://192.168.1.5:5500/privado/pages/${currentPage}.js`,{}, "text")    
        const retonarPaginaOcultaEmFunção = Function("return " + pegarPaginaOculta)();   
         
        const { html, state, style } = await retonarPaginaOcultaEmFunção(paginasData)   
        const renderStyle = typeof style === "function" ? style : undefined;
        const renderedHtml = typeof html === "function" ? html() : html()
        const renderState = typeof state === "function" ? state : undefined; 
        customTagsPrivate(renderStyle, renderedHtml, renderState);
...
await Promise.all(
  tagElements.map(async (elem, i) => {
    ...
    if (componentCache.hasOwnProperty(componentKey)) {
      componentResult = componentCache[componentKey];
    } else {
      if (components.hasOwnProperty(componentKey)) {
        const { style, html, state } = await components[componentKey](dataApp);
        componentResult = { style, html, state };
        componentCache[componentKey] = componentResult;
      } else {
        throw new Error(`Componente não encontrado para a tag: ${componentKey}`);
      }
    }

    if (typeof componentResult.style === "function") {
      stateStyles.push(componentResult.style);
    }
    if (typeof componentResult.html === "function") {
      newTag.innerHTML = componentResult.html();
    }
    if (typeof componentResult.state === "function") {
      stateFunctions.push(componentResult.state);
    }
  })
);
