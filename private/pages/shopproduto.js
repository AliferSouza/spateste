async function produto(props) {  
    const dadosSelecionado = props.Dados.produtos.filter(d => d.categoria === location.hash.split("#").pop());
    const dados = props.Dados.produtos.filter(d => d.categoria === "oleos") || dadosSelecionado 
            ...
            <div class="perfumaria-container-cards">
                ${dados.map((produto, key) => `
                    <div class="perfumaria-container-card" id="${produto.id}">           
                        <img data-url-src="${produto.img}" width="150">
                        <h4>${produto.nome}</h4>
                        <div>
